...
public:
	...
	int HRA, PF, tax, LoanBalance, LoanDebit, grosspay, workingHours, DA,MealAllowance, TransportAllowance, MedicalAllowance;
...
void getrecords()
{
	...
	if(fp!=NULL)
	{
		while(feof(fp)==0)
		{
			fscanf(fp,"%s\t%d\t%s\t%d\t%d\t%d\t%c\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",&emp[c].name,&emp[c].code,&emp[c].designation,&emp[c].exp,&emp[c].age,&emp[c].salary,&emp[c].AnyLoan,&emp[c].HRA,&emp[c].PF,&emp[c].tax,&emp[c].LoanBalance,&emp[c].LoanDebit,&emp[c].grosspay,&emp[c].workingHours,&emp[c].DA,&emp[c].MealAllowance,&emp[c].TransportAllowance,&emp[c].MedicalAllowance);
			c++;
		}
		num=c;
	}
...
void saverecords()
{
	...
	else
	{
		...
		for(int i=0;i<num;i++)
		{
			fprintf(fp,"%s\t%d\t%s\t%d\t%d\t%d\t%c\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",emp[i].name,emp[i].code,emp[i].designation,emp[i].exp,emp[i].age,emp[i].salary,emp[i].AnyLoan,emp[i].HRA,emp[i].PF,emp[i].tax,emp[i].LoanBalance,emp[i].LoanDebit,emp[i].grosspay,emp[i].workingHours,emp[i].DA,emp[i].MealAllowance,emp[i].TransportAllowance,emp[i].MedicalAllowance);
		}
		fclose(fp);
	}
}
...
void login()
{
	...
	Password[i] = '\0';
    if(strcmp(UserName,"admin")==0 && strcmp(Password,"aryan")==0)
    {
    	system("cls");
    	loginFrame1();
    	gotoXY(27,10);
...
void insert()
{
	borderNoDelay();
	int i=num;
	int sal,PF,TAX,HRA,LoanBal,LoanDeb,h,DA,MealA,MedicalA,TransportA;
	char loan;
	num+=1;
	gotoXY(28,4);
	...
	emp[i].grosspay = (sal+HRA+MealA+MedicalA+TransportA+DA)-(PF+TAX+LoanDeb) ;
	getch();
}
void intro()
{
	gotoXY(27,4); printf("PAYROLL MANAGEMENT SYSTEM");
    gotoXY(24,20);printf("Press any buttom to continue...");
    getch();
...
{
	int sal,PF,TAX,HRA,LoanBal=emp[i].LoanBalance,LoanDeb,DA,MealA,MedicalA,TransportA;
	char loan;
	gotoXY(10,18);
...
void displayPayslip()
{
	...
	for(i=0;i<=num-1;i++)
	{
		if(emp[i].code==code)
		{
			...
			gotoXY(8,12);
			cout<<"HRA(24% of Basic): "<<emp[i].HRA<<endl;
			gotoXY(8,14);
			cout<<"DA (140% of Basic):"<<emp[i].DA;
			gotoXY(8,16);
